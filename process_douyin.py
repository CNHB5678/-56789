...
def download_video(url, output_folder, cookies_path=None):
    ...
    ydl_opts = {
        ...
        'subtitleslangs': ['zh-Hans', 'zh', 'zh-CN'],
    ...
    try:
        ...
        print(f"下载失败: {e}")
        print("\n提示: 可能需要更新cookies文件")
        print("请查看 /workspace/README.md 了解如何获取cookies")
        return None, None
...
def main():
    ...
    # 下载视频
    info_dict, video_filename = download_video(video_url, output_folder, cookies_file)
    
    if not video_filename or not os.path.exists(video_filename):
        print("\n无法下载视频，可能需要cookies文件")
        print("请查看 /workspace/README.md 了解如何获取cookies")
        return
